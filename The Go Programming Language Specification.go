...
Type unification
类型推断基于类型统一。单个统一步骤适用于替换映射和两种类型，其中一种或两种类型可以是或包含类型参数。替换映射跟踪已知的（明确提供的或已经推断出
的）类型参数：该映射包含一个条目 P → A 用于每个类型参数 P 和相应的已知类型参数 A。在统一期间，已知类型参数取代它们相应的类型参数比较类型时。
统一是找到使两种类型等价的替换映射条目的过程。

对于统一，不包含当前类型参数列表中的任何类型参数的两个类型是等价的，如果它们相同，或者如果它们是相同的通道类型而忽略通道方向，或者如果它们的基础类型是等价的。

统一通过比较类型对的结构来实现：它们的结构（不考虑类型参数）必须相同，并且类型参数以外的类型必须等价。一种类型中的类型参数可以匹配另一种类型中
的任何完整子类型；每个成功的匹配都会导致一个条目被添加到替换映射中。如果结构不同，或者类型参数以外的类型不等价，则统一失败。
...
通过将向量的类型与相应的参数类型统一起来，可以从函数参数向量中推断出 Number 的类型参数：[]float64 和 []Number 在结构中匹配，float64 与
Number 匹配。这会将条目 Number → float64 添加到替换映射中。无类型参数，比如这里的第二个函数参数 42，在第一轮函数参数类型推断中被忽略，只
有在还有未解析的类型参数时才会考虑。
...
通过构造，Lu 中对的参数是无类型常量（或比较的无类型布尔结果）。并且因为无类型值的默认类型总是预先声明的非复合类型，它们永远不能与复合类型匹配，
所以只考虑单一类型参数的参数类型就足够了。
...
在示例 min(1.0, 2) 中，处理函数参数 1.0 会产生替换映射条目 T → float64。因为处理会一直持续到所有未类型化的参数都被考虑在内，所以会报告错
误。这确保类型推断不依赖于未类型化参数的顺序。

Constraint type inference
约束类型推断通过考虑类型约束来推断类型参数。如果类型参数 P 具有核心类型 C 的约束，则将 P 与 C 统一可以推断出其他类型参数，或者是 P 的类型参
数，或者如果已知，则可能是 C 中使用的类型参数的类型参数。

例如，考虑带有类型参数 List 和 Elem 的类型参数列表：
[List ~[]Elem, Elem any]

约束类型推断可以从List的类型参数推导出Elem的类型，因为Elem是List的核心类型[]Elem中的类型参数。如果类型参数是 byte：
type Bytes []byte

将 Bytes 的底层类型与核心类型统一意味着将 []byte 与 []Elem 统一。该统一成功并产生替换映射条目 Elem → byte。因此，在这个例子中，约束类型
推断可以从第一个参数推断出第二个类型参数。

使用约束的核心类型可能会丢失一些信息：在（不太可能）约束的类型集包含单个定义类型 N 的情况下，相应的核心类型是 N 的基础类型而不是 N 本身。在
这种情况下，约束类型推断可能成功但实例化将失败，因为推断类型不在约束的类型集中。因此，约束类型推断使用调整后的约束核心类型：如果类型集包含单个
类型，则使用该类型；否则，使用该类型。否则使用约束的核心类型。

对于调整后的核心类型的所有类型参数，将类型参数与该类型统一。如果任何统一失败，则约束类型推断失败。
	1.对于调整后的核心类型的所有类型参数，将类型参数与该类型统一。如果任何统一失败，则约束类型推断失败。
	2.此时，M 中的某些条目可能会将类型参数映射到其他类型参数或映射到包含类型参数的类型。对于 M 中的每个条目 P → A，其中 A 是或包含类型参数
	Q，并且在 M 中存在条目 Q → B，将这些 Q 替换为 A 中的相应 B。当无法进一步替换时停止。
...
shift 表达式中的右操作数必须具有整数类型，或者是由 uint 类型的值表示的无类型常量。如果非常量移位表达式的左操作数是无类型常量，则首先将其隐式
转换为移位表达式单独由其左操作数替换时所假定的类型。
var a [1024]byte
var s uint = 33
...
移位运算符将左操作数移动右操作数指定的移位计数，该计数必须为非负数。如果轮班计数在运行时为负数，则会发生运行时恐慌。如果左操作数是有符号整数，
则移位运算符执行算术移位，如果它是无符号整数，则执行逻辑移位。轮班次数没有上限。移位的行为就好像左操作数按 1 移位 n 次，移位计数为 n。结果，
x << 1 与 x*2 相同，x >> 1 与 x/2 相同，但截断为负无穷大。
...
Integer overflow
对于无符号整数值，操作 +、-、* 和 << 以 2n 为模计算，其中 n 是无符号整数类型的位宽。粗略地说，这些无符号整数运算在溢出时丢弃高位，程序可能
依赖于“环绕”。

对于有符号整数，操作 +、-、*、/ 和 << 可能会合法溢出，并且结果值存在并且由有符号整数表示、操作及其操作数确定地定义。溢出不会导致运行时恐慌。
假设不会发生溢出，编译器可能不会优化代码。例如，它可能不会假定 x < x + 1 始终为真。

Floating-point operators
对于浮点数和复数，+x 与 x 相同，而 -x 是 x 的否定。浮点数或复数除以零的结果未超出 IEEE-754 标准；是否发生运行时恐慌是特定于实现的。

一个实现可以将多个浮点运算组合成一个单一的融合运算，可能跨语句，并产生一个不同于通过单独执行和舍入指令获得的值的结果。显式浮点类型转换舍入到目
标类型的精度，防止会丢弃该舍入的融合。
...
Address operators
对于类型 T 的操作数 x，地址操作 &x 生成指向 x 的 *T 类型的指针。操作数必须是可寻址的，即变量、指针间接或切片索引操作；或可寻址结构操作数的字
段选择器；或可寻址数组的数组索引操作。作为可寻址要求的一个例外，x 也可以是一个（可能带括号的）复合文字。如果 x 的评估会导致运行时恐慌，那么 &x
的评估也会。
...
Receive operator
对于核心类型为通道的操作数ch，接收操作的值<-ch是从通道ch接收到的值。通道方向必须允许接收操作，接收操作的类型是通道的元素类型。表达式阻塞，直
到值可用。从 nil 通道接收永远阻塞。关闭通道上的接收操作始终可以立即进行，在接收到任何先前发送的值后产生元素类型的零值。
v1 := <-ch
v2 = <-ch
f(<-ch)
...
var x, ok = <-ch
var x, ok T = <-ch
产生一个额外的无类型布尔结果，报告通信是否成功。如果接收到的值是通过成功的发送操作传递到通道的，则 ok 的值为 true；如果它是由于通道关闭且为空
而生成的零值，则为 false。
...
转换 P(1.1) 导致类型 P 的非常量值，值 1.1 表示为 float32 或 float64，具体取决于 f 的类型参数。因此，如果 f 使用 float32 类型实例化，则
表达式 P(1.1) + 1.2 的数值将使用与相应的非常量 float32 加法相同的精度计算。
...
对于非常量数值的转换，适用以下规则：
	1.在整数类型之间转换时，如果值为有符号整数，则将其符号扩展为隐式无限精度；否则为零扩展。然后将其截断以适合结果类型的大小。例如，如果 v
:= uint16(0x10F0)，则 uint32(int8(v)) == 0xFFFFFFF0。转换总是产生有效值；没有溢出的迹象。
	2.将浮点数转换为整数时，分数将被丢弃（截断为零）。
	3.将整数或浮点数转换为浮点类型，或将复数转换为另一种复数类型时，结果值四舍五入为目标类型指定的精度。例如，可以使用超出 IEEE-754 32 位数
字精度的额外精度存储类型为 float32 的变量 x 的值，但 float32(x) 表示将 x 的值四舍五入为 32 位精度的结果。同样，x + 0.1可能会使用超过 32
位的精度，但 float32(x + 0.1) 不会。
...
对无类型常量的任何其他操作都会产生同类无类型常量；即，布尔值、整数、浮点数、复数或字符串常量。如果二元运算（移位除外）的无类型操作数属于不同种
类，则结果属于此列表后面出现的操作数种类：整数、符文、浮点数、复数。例如，一个无类型整型常量除以一个无类型复数常量得到一个无类型复数常量。
...
实现限制：编译器可以在计算无类型浮点或复杂常量表达式时使用舍入；请参阅常量部分中的实现限制。这种舍入可能导致浮点常量表达式在整数上下文中无效，
即使在使用无限精度计算时它是整数，反之亦然。

Order of evaluation
在包级别，初始化依赖关系确定变量声明中各个初始化表达式的计算顺序。否则，在计算表达式、赋值或返回语句的操作数时，所有函数调用、方法调用和通信操
作都按从左到右的词法顺序计算。
...
在通信开始之前评估通道和值表达式。通信阻塞，直到发送可以继续。如果接收方准备就绪，则可以继续进行无缓冲通道上的发送。如果缓冲区中有空间，则缓冲
通道上的发送可以继续。关闭通道上的发送会导致运行时恐慌。在 nil 通道上发送永远阻塞。在通信开始之前评估通道和值表达式。通信阻塞，直到发送可以继
续。如果接收方准备就绪，则可以继续进行无缓冲通道上的发送。如果缓冲区中有空间，则缓冲通道上的发送可以继续。关闭通道上的发送会导致运行时恐慌。在
nil 通道上发送永远阻塞。
...
赋值运算 x op= y 其中 op 是二元算术运算符等同于 x = x op (y) 但仅对 x 求值一次。 op= 构造是单个标记。在赋值操作中，左边和右边的表达式列
表都必须只包含一个单值表达式，并且左边的表达式不能是空白标识符。
a[i] <<= 2
i &^= 1<<n

元组分配将多值运算的各个元素分配给变量列表。有两种形式。首先，右手操作数是单个多值表达式，例如函数调用、通道或映射操作或类型断言。左侧操作数的
数量必须与值的数量匹配。例如，如果 f 是一个返回两个值的函数，
x, y = f()
将第一个值分配给 x，将第二个值分配给 y。在第二种形式中，左边的操作数的个数必须等于右边的表达式的个数，每个表达式都必须是单值的，右边的第n个表
达式赋值给左边的第n个操作数：
one, two, three = '一', '二', '三'
...
任务分两个阶段进行。首先，左边的索引表达式和指针间接（包括选择器中的隐式指针间接）和右边的表达式的操作数都按通常的顺序求值。其次，分配是按从左
到右的顺序进行的。
...
有两种形式：表达式开关和类型开关。在表达式开关中，案例包含与开关表达式的值进行比较的表达式。在类型开关中，案例包含与特殊注释的开关表达式的类型
进行比较的类型。 switch 表达式在 switch 语句中只计算一次。

Expression switches
在表达式 switch 中，switch 表达式被求值，case 表达式不需要是常量，从左到右和从上到下求值；第一个等于 switch 表达式的触发执行关联案例的语
句；其他情况被跳过。如果没有匹配的案例并且存在“默认”案例，则执行其语句。最多可以有一个 default case，它可以出现在“switch”语句的任何地方。
缺少 switch 表达式等同于布尔值 true。

ExprSwitchStmt = "switch" [ SimpleStmt ";" ] [ Expression ] "{" { ExprCaseClause } "}" .
ExprCaseClause = ExprSwitchCase ":" StatementList .
ExprSwitchCase = "case" ExpressionList | "default" .

如果 switch 表达式的计算结果为无类型常量，则首先将其隐式转换为其默认类型。预先声明的无类型值 nil 不能用作 switch 表达式。 switch 表达式类
型必须是可比较的。
...
在 case 或 default 子句中，最后一个非空语句可能是（可能标记为）“fallthrough”语句，以指示控制应从该子句的末尾流向下一个子句的第一个语句。
否则控制流到“switch”语句的末尾。 “fallthrough”语句可能作为表达式开关的最后一个子句以外的所有语句的最后一个语句出现。
...
然后用例将实际类型 T 与表达式 x 的动态类型进行匹配。与类型断言一样，x 必须是接口类型，但不是类型参数，并且 case 中列出的每个非接口类型 T 都
必须实现 x 的类型。在类型开关的情况下列出的类型必须全部不同。
TypeSwitchStmt  = "switch" [ SimpleStmt ";" ] TypeSwitchGuard "{" { TypeCaseClause } "}" .
TypeSwitchGuard = [ identifier ":=" ] PrimaryExpr "." "(" "type" ")" .
TypeCaseClause  = TypeSwitchCase ":" StatementList .
TypeSwitchCase  = "case" TypeList | "default" .

TypeSwitchGuard 可能包含一个简短的变量声明。使用该形式时，变量在每个子句的隐式块中的 TypeSwitchCase 末尾声明。在 case 中仅列出一种类型的
子句中，变量具有该类型；否则，该变量具有 TypeSwitchGuard 中表达式的类型。
...
带有 ForClause 的“for”语句也由其条件控制，但另外它可以指定 init 和 post 语句，例如赋值、增量或减量语句。 init 语句可以是一个简短的变量声
明，但 post 语句不能。 init 语句声明的变量在每次迭代中重复使用。
...
如果非空，则在第一次迭代评估条件之前执行一次 init 语句； post 语句在每次块执行后执行（并且仅当块被执行时）。 ForClause 的任何元素都可以为空
，但分号是必需的，除非只有一个条件。如果条件不存在，则相当于布尔值 true。
for cond { S() }    is the same as    for ; cond ; { S() }
for      { S() }    is the same as    for true     { S() }

For statements with range clause
带有“range”子句的“for”语句遍历数组、切片、字符串或映射的所有条目，或通道上接收的值。对于每个条目，它将迭代值分配给相应的迭代变量（如果存在）
，然后执行该块。
RangeClause = [ ExpressionList "=" | IdentifierList ":=" ] "range" Expression .

“范围”子句右边的表达式称为范围表达式，其核心类型必须是数组、指向数组、切片、字符串、映射或允许接收操作的通道的指针。与赋值一样，如果存在，则左
侧的操作数必须是可寻址的或映射索引表达式；它们表示迭代变量。如果范围表达式是通道，则最多允许一个迭代变量，否则最多可以有两个。如果最后一个迭代
变量是空白标识符，则范围子句等效于没有该标识符的相同子句。
...
迭代变量可以使用短变量声明 (:=) 的形式由“范围”子句声明。在这种情况下，它们的类型被设置为各自迭代值的类型，它们的范围是“for”语句的块；它们在
每次迭代中都被重新使用。如果迭代变量在“for”语句之外声明，执行后它们的值将是最后一次迭代的值。
var testdata *struct {
	a *[7]int
}
for i, _ := range testdata.a {
	// testdata.a 永远不会被计算； len(testdata.a) 是常数 i 范围从 0 到 6
	// testdata.a is never evaluated; len(testdata.a) is constant i ranges from 0 to 6
	f(i)
}
...
函数值和参数在调用 goroutine 中像往常一样被评估，但与常规调用不同，程序执行不会等待调用的函数完成。相反，该函数开始在新的 goroutine 中独立
执行。当函数终止时，它的 goroutine 也终止。如果函数有任何返回值，它们将在函数完成时被丢弃。
go Server()
...
具有 RecvStmt 的情况可以将 RecvExpr 的结果分配给一个或两个变量，这些变量可以使用短变量声明来声明。 RecvExpr 必须是（可能带括号的）接收操
作。最多可以有一个默认案例，它可能出现在案例列表中的任何位置。

“select”语句的执行分几个步骤进行：
1.对于语句中的所有情况，接收操作的通道操作数以及发送语句的通道和右侧表达式在输入“select”语句时按源顺序恰好计算一次。结果是一组要从中接收或发
送到的通道，以及要发送的相应值。无论选择哪个（如果有的话）通信操作继续进行，该评估中的任何副作用都会发生。 RecvStmt 左侧带有简短变量声明或赋
值的表达式尚未计算。
2.如果一个或多个通信可以继续进行，则通过统一的伪随机选择选择一个可以继续进行的通信。否则，如果存在默认情况，则选择该情况。如果没有默认情况，则
“select”语句将阻塞，直到至少有一个通信可以继续进行。
...
2.“返回”语句中的表达式列表可能是对多值函数的单次调用。效果就好像从该函数返回的每个值都被分配给一个具有相应值类型的临时变量，然后是列出这些变量
的“return”语句，此时应用前一个案例的规则。
...
每次执行“defer”语句时，调用的函数值和参数都会像往常一样计算并重新保存，但不会调用实际函数。相反，延迟函数会在周围函数返回之前立即被调用，顺序
与它们被延迟的顺序相反。也就是说，如果周围函数通过显式 return 语句返回，则延迟函数将在该 return 语句设置任何结果参数之后但在函数返回其调用者
之前执行。如果延迟函数的值计算为 nil，则在调用该函数时执行恐慌，而不是在执行“defer”语句时发生恐慌。

例如，如果延迟函数是一个函数文字，并且周围的函数在文字范围内命名了结果参数，则延迟函数可以在返回之前访问和修改结果参数。如果延迟函数有任何返回
值，它们在函数完成时被丢弃。 （另请参阅有关处理恐慌的部分。）
lock(l)
...
Close
对于核心类型为通道的参数 ch，内置函数 close 记录将不会在通道上发送更多值。如果 ch 是只接收通道，则会出错。发送或关闭关闭的通道会导致运行时恐
慌。关闭 nil 通道也会导致运行时恐慌。在调用 close 之后，并且在接收到之前发送的任何值之后，接收操作将返回通道类型的零值而不会阻塞。多值接收操
作返回接收值以及通道是否关闭的指示。
...
每个大小参数 n 和 m 都必须是整数类型，具有仅包含整数类型的类型集，或者是无类型常量。常量大小参数必须是非负的并且可以用 int 类型的值表示；如果
它是一个无类型常量，它被赋予类型 int。如果同时提供 n 和 m 并且它们是常量，则 n 必须不大于 m。对于切片和通道，如果运行时 n 为负数或大于 m，
则会发生运行时恐慌。
...
可变参数函数 append 将零个或多个值 x 附加到切片 s 并返回与 s 类型相同的结果切片。 s 的核心类型必须是 []E 类型的切片。值 x 被传递给类型为
...E 的参数，并且应用相应的参数传递规则。作为一种特殊情况，如果 s 的核心类型是 []byte，append 还接受第二个核心类型 bytestring 后跟 ...
的参数。这种形式附加字节切片或字符串的字节。
...
函数 copy 将切片元素从源 src 复制到目标 dst 并返回复制的元素数。两个参数的核心类型必须是具有相同元素类型的切片。复制的元素数是 len(src)和
len(dst)中的最小值。作为一种特殊情况，如果目标的核心类型是 []byte，则复制还接受具有核心类型 bytestring 的源参数。这种形式将字节切片或字符
串中的字节复制到字节切片中。
copy(dst, src []T) int
...
参数类型和返回值对应。对于复数，两个参数必须是相同的浮点类型，并且返回类型是具有相应浮点成分的复数类型：float32 参数为 complex64，float64
参数为  complex128。如果其中一个参数的计算结果为无类型常量，则首先将其隐式转换为另一个参数的类型。如果两个参数的计算结果都是无类型常量，则它
们必须是非复数或它们的虚部必须为零，并且函数的返回值是无类型复常量。

对于 real 和 imag，参数必须是复数类型，返回类型是对应的浮点类型：complex64 参数为 float32，complex128 参数为 float64。如果参数的计算结
果为无类型常量，则它必须是数字，并且函数的返回值是无类型浮点常量。
...
recover 函数允许程序管理 panicing goroutine 的行为。假设一个函数 G 推迟了一个调用 recover 的函数 D，并且在执行 G 的同一个 goroutine
上的一个函数中发生了恐慌。当 deferred 函数运行到 D 时，D 调用 recover 的返回值将是传递给 panic 调用的值。如果 D 正常返回，没有开始新的 panic
，则 panic 序列停止。在这种情况下，在 G 和调用 panic 之间调用的函数的状态将被丢弃，并恢复正常执行。然后运行由 G 在 D 之前延迟的任何函数，
并且 G 的执行通过返回其调用者而终止。
...
print      prints all arguments; formatting of arguments is implementation-specific		打印所有参数；参数的格式是特定于实现的
println    like print but prints spaces between arguments and a newline at the end      像 print 但打印参数之间的空格和末尾的换行符
实现限制：print 和 println 不需要接受任意参数类型，但必须支持 boolean、numeric 和 string 类型的打印。

Packages
Go 程序是通过将包链接在一起构建的。一个包又是由一个或多个源文件构成的，这些源文件一起声明属于该包的常量、类型、变量和函数，并且可以在同一包的
所有文件中访问。这些元素可以导出并在另一个包中使用。

Source file organization
每个源文件都包含一个 package 子句，定义它所属的包，然后是一组可能为空的导入声明，这些声明声明它希望使用其内容的包，然后是一组可能为空的函数
、类型、变量声明，和常量。
...
Import declarations
导入声明声明包含声明的源文件取决于导入包的功能（§程序初始化和执行），并允许访问该包的导出标识符。导入命名用于访问的标识符 (PackageName) 和指
定要导入的包的 ImportPath。
ImportDecl       = "import" ( ImportSpec | "(" { ImportSpec ";" } ")" ) .
ImportSpec       = [ "." | PackageName ] ImportPath .
ImportPath       = string_lit .

PackageName 在限定标识符中使用，以访问导入源文件中包的导出标识符。它在文件块中声明。如果省略 PackageName，则默认为导入包的 package 子句中
指定的标识符。如果出现显式句点 (.) 而不是名称，则在该包的包块中声明的所有包的导出标识符都将在导入源文件的文件块中声明，并且必须在没有限定符的
情况下访问。

ImportPath 的解释依赖于实现，但它通常是已编译包的完整文件名的子字符串，并且可能与已安装包的存储库相关。

实现限制：编译器可以限制 ImportPaths 为非空字符串，只使用属于 Unicode 的 L、M、N、P 和 S 一般类别的字符（不带空格的图形字符），也可以排除
字符！"#$%& '()*,:;<=>?[\]^`{|} 和 Unicode 替换字符 U+FFFD。

考虑一个包含包子句 package math 的编译包，它导出函数 Sin，并将编译包安装在由“lib/math”标识的文件中。此表说明了在各种类型的导入声明之后导
入包的文件中如何访问 Sin。
...
导入声明声明导入和导入包之间的依赖关系。包直接或间接导入自身，或直接导入包而不引用其任何导出的标识符都是非法的。要仅为其副作用（初始化）导入包
，请使用空白标识符作为显式包名称：
import _ "lib/math"
...
The zero value
当通过声明或调用 new 为变量分配存储空间时，或者通过复合文字或调用 make 创建新值时，并且没有提供显式初始化，则给出变量或值一个默认值。此类变
量或值的每个元素都设置为其类型的零值：布尔值为 false，数字类型为 0，字符串为 ""，指针、函数、接口、切片、通道和映射为 nil。此初始化是递归完
成的，因此，例如，如果未指定值，则结构数组的每个元素都将其字段清零。
...
更准确地说，如果包级变量尚未初始化并且没有初始化表达式或其初始化表达式不依赖于未初始化的变量，则认为包级变量已准备好进行初始化。初始化通过重复
初始化声明顺序中最早并准备初始化的下一个包级变量来进行，直到没有变量准备好进行初始化。
...
依赖分析不依赖于变量的实际值，仅依赖于源中对它们的词法引用，进行传递分析。例如，如果变量 x 的初始化表达式引用一个函数，其函数体引用变量 y，则
x 依赖于 y。具体来说：{
...
每个包都可以定义多个这样的函数，即使在单个源文件中也是如此。在 package 块中，init 标识符只能用于声明 init 函数，但不声明标识符本身。因此，
不能在程序的任何地方引用 init 函数。

一个没有导入的包是通过为它的所有包级变量分配初始值，然后按照它们在源代码中出现的顺序调用所有 init 函数来初始化的，可能在多个文件中，如呈现给
编译器。如果一个包有导入，导入的包在初始化包本身之前被初始化。如果多个包导入一个包，导入的包只会初始化一次。包的导入，通过构造，保证不会有循环
初始化依赖。

包初始化——变量初始化和 init 函数的调用——在单个 goroutine 中依次发生，一次一个包。 init 函数可以启动其他 goroutine，它们可以与初始化代码
同时运行。但是，初始化总是对 init 函数进行排序：它不会调用下一个函数，直到前一个函数返回。

为了确保可重现的初始化行为，鼓励构建系统以词法文件名顺序向编译器呈现属于同一包的多个文件。

Program execution
一个完整的程序是通过将称为主包的单个未导入的包与其导入的所有包可传递地链接起来创建的。主包必须具有包名 main 并声明一个不带参数且不返回任何值
的函数 main。
func main() { … }
...
Run-time panics
执行错误（例如尝试越界索引数组）会触发运行时恐慌，这等同于使用实现定义的接口类型 runtime.Error 的值调用内置函数恐慌。该类型满足预先声明的接
口类型错误。未指定表示不同运行时错误条件的确切错误值。
package runtime
...
Package unsafe
编译器已知并可通过导入路径“unsafe”访问的内置包 unsafe 为低级编程提供了便利，包括违反类型系统的操作。使用 unsafe 的包必须手动检查类型安全，
并且可能不可移植。该软件包提供以下接口：
package unsafe
...
Pointer 是一种指针类型，但不能取消引用 Pointer 值。底层类型 uintptr 的任何指针或值都可以转换为底层类型 Pointer 的类型，反之亦然。 Pointer
和 uintptr 之间的转换效果是实现定义的。
var f float64
...
函数 Offsetof 采用（可能带括号的）选择器 s.f，表示由 s 或 *s 表示的结构的字段 f，并返回相对于结构地址的字段偏移量（以字节为单位）。如果 f
是一个嵌入式字段，则它必须可以通过结构的字段在没有指针间接访问的情况下访问。对于具有字段 f 的结构 s：
uintptr(unsafe.Pointer(&s)) + unsafe.Offsetof(s.f) == uintptr(unsafe.Pointer(&s.f))

计算机体系结构可能需要对齐内存地址；也就是说，对于一个变量的地址是一个因子的倍数，变量的类型对齐。函数 Alignof 采用表示任何类型变量的表达式，
并以字节为单位返回变量（的类型）的对齐方式。对于变量 x：
uintptr(unsafe.Pointer(&x)) % unsafe.Alignof(x) == 0

如果 T 是类型参数，或者如果它是包含可变大小的元素或字段的数组或结构类型，则类型 T 的（变量）具有可变大小。否则大小不变。对 Alignof、Offsetof
和 Sizeof 的调用是 uintptr 类型的编译时常量表达式，前提是它们的参数（或 Offsetof 的选择器表达式 s.f 中的结构 s）是常量大小的类型。

函数 Add 将 len 添加到 ptr 并返回更新后的指针 unsafe.Pointer(uintptr(ptr) + uintptr(len))。 len 参数必须是整数类型或无类型常量。常
量 len 参数必须可以用 int 类型的值表示；如果它是一个无类型常量，它被赋予类型 int。有效使用 Pointer 的规则仍然适用。

函数 Slice 返回一个切片，其底层数组从 ptr 开始，长度和容量为 len。 Slice(ptr, len) 等价于
(*[len]ArbitraryType)(unsafe.Pointer(ptr))[:]
除了作为一种特殊情况，如果 ptr 为 nil 且 len 为零，则 Slice 返回 nil。

len 参数必须是整数类型或无类型常量。常量 len 参数必须是非负的并且可以用 int 类型的值表示；如果它是一个无类型常量，它被赋予类型 int。在运行
时，如果 len 为负，或者 ptr 为 nil 且 len 不为零，则会发生运行时恐慌。
